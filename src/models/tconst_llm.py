...
class QkvGetBlock(nn.Module, CacheMixin):
    ...
    def forward(self, normed_x_proj, cache_key="x_qkv"):
        ...
        if gen_flag:
            ...
            if cache_on and (cache_key != "context_origin_qkv"):
...
class TConstFormer(nn.Module):
    ...
    def get_history_window_start_pos(self, context_origin):
        ...
        if history_windown_size == "auto":
            ...
            # (such as the n > C log N rule), C~= 8.33
            # here we still pad to 
            history_windown_size = 8.33 * math.log(l_context_origin)
...
# ======================================================================================
# pylint: disable=too-many-instance-attributes
class TConstLLM(nn.Module, CacheMixin):
